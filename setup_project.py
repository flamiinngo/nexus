...
async def _create_payment_header(payment_requirements: dict) -> str | None:
    ...
    if not client:
        return None
    try:
        # The 402 response contains the payment requirements
        # x402 SDK creates a signed payment payload
        payload = await client.create_payment_payload(payment_requirements)
        if payload and http_client:
            # Encode the payment payload as the header value
            import json, base64
