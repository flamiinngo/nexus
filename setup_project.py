...
async def _create_payment_header(payment_requirements: dict) -> str | None:
    ...
    if not client:
        return None
    try:
        # The 402 response is a raw dict — we need to convert it to what the SDK expects
        # The SDK's create_payment_payload expects the payment requirements object
        # We pick the first "accepts" entry that matches Base (chain 8453)
        accepts = payment_requirements.get("accepts", [])
        if not accepts:
            logger.error("No payment options in 402 response")
            return None

        # Prefer Base (8453), fallback to first option
        selected = None
        for a in accepts:
        ...
        # The x402 SDK's create_payment_payload expects the FULL 402 response object
        # which has x402Version and accepts[] — not a single requirement
        # We need to construct a proper PaymentRequired object
        try:
            # Try passing the full 402 response dict — the SDK should handle it
            # The response has: {"x402Version": 2, "accepts": [...], "error": "..."}
            # SDK expects an object with .x402_version attribute
            # Let's create a simple wrapper
            class PaymentRequired:
                def __init__(self, data):
                    self.x402_version = data.get("x402Version", 2)
                    self.accepts = data.get("accepts", [])
                    self.error = data.get("error", "")
                    # Make individual accepts accessible as objects too
                    self._accepts_objects = []
                    for a in self.accepts:
                        obj = type("Req", (), {
                            ...
                            "amount": a.get("amount"),
                            "pay_to": a.get("payTo"),
                            "max_timeout_seconds": a.get("maxTimeoutSeconds", 60),
                            "extra": a.get("extra", {}),
                        })()
                        self._accepts_objects.append(obj)

            pr = PaymentRequired(payment_requirements)
            payload = await client.create_payment_payload(pr)
        except Exception as e1:
            logger.error("SDK with wrapper failed: %s", e1)
            # Fallback: try with the raw dict
            try:
                payload = await client.create_payment_payload(payment_requirements)
            except Exception as e2:
                logger.error("SDK with raw dict also failed: %s", e2)
                payload = None
        ...
        # The payload needs to be base64 encoded for the header
