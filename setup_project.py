...
def _extract_text(data) -> str:
    ...
    if isinstance(data, str):
        ...
        m = re.search(r'"text"\\s*:\\s*"(.*?)"', data[:2000])
        if m:
            try:
                return json.loads(f'"{m.group(1)}"')[:800]
            except:
                pass
        # Try to find JSON in SSE data lines
        m2 = re.search(r'data:\\s*(\\{.*?\\})', data[:3000], re.DOTALL)
        if m2:
            try:
                outer = json.loads(m2.group(1))
                inner_text = outer.get("result", {}).get("content", [{}])[0].get("text", "")
                if inner_text:
                    try:
                        parsed = json.loads(inner_text)
                        return json.dumps(parsed, indent=0)[:800]
                    except:
    ...
    if isinstance(data, dict):
        ...
        return json.dumps(data, default=str, indent=0)[:800]
    return str(data)[:500]
