...
def _plan_token_eval(goal):
    return ExecutionPlan(goal=goal, task_type=TaskType.TOKEN_EVALUATION, execution_mode=ExecutionMode.PARALLEL, subtasks=[
        Subtask(id="security", agent_name="quill", tool_name="search_token", description="Analyze token contract security", parameters={"query": goal}),
...
def _plan_opportunity(goal):
    return ExecutionPlan(goal=goal, task_type=TaskType.OPPORTUNITY_DISCOVERY, execution_mode=ExecutionMode.MIXED, subtasks=[
        ...
        Subtask(id="security_check", agent_name="quill", tool_name="search_token", description="Security check discoveries", parameters={"query": goal}, depends_on=["search"]),
        ...
        try:
            ...
            if resp.status_code == 402:
                try:
                    ...
                    pay_data = {"raw": resp.text[:300]}
                return SubtaskResult(subtask_id=subtask.id, agent_name=subtask.agent_name, success=False, data={"payment_required": pay_data}, error="x402 payment required", execution_time_ms=ms)
