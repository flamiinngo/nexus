...
async def _create_payment_header(payment_requirements: dict) -> str | None:
    ...
    try:
        ...
        # Build payment requirements in the format the SDK expects
        # The x402 client.create_payment_payload expects a list of requirement dicts
        # matching the "accepts" format from the 402 response
        requirement = {
            ...
            "maxTimeoutSeconds": selected.get("maxTimeoutSeconds", 60),
            "extra": selected.get("extra", {}),
        }

        # Try the SDK's create_payment_payload with the full 402 response
        try:
            # x402 v2 SDK expects the full payment_required object
            ...
            pr = PaymentRequirements(**{
                ...
                "max_timeout_seconds": selected.get("maxTimeoutSeconds", 60),
                "extra": selected.get("extra", {}),
            })
            payload = await client.create_payment_payload(pr)
        except ImportError:
            # Fallback: try passing the requirement dict directly
            logger.info("PaymentRequirements import failed, trying dict directly")
            try:
                payload = await client.create_payment_payload([requirement])
            except Exception:
                payload = await client.create_payment_payload(requirement)
