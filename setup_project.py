...
async def _create_payment_header(payment_requirements: dict) -> str | None:
    ...
    try:
            ...
            def __init__(self, d):
                for k, v in d.items():
                    ...
                    if isinstance(v, dict):
                        setattr(self, sk, AttrDict(v))
                        setattr(self, k, AttrDict(v))  # also keep original key
                    elif isinstance(v, list):
                        converted = [AttrDict(i) if isinstance(i, dict) else i for i in v]
        ...
        # Ensure accepts items have both camelCase and snake_case attrs
